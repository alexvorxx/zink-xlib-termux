...
fn legalize_sm70_instr(b: &mut LegalizeBuilder, instr: &mut Instr) {
    ...
    if !matches!(&instr.op, Op::Ldc(_)) && instr.is_uniform() {
        ...
        let src_types = instr.src_types();
        for (i, src) in instr.srcs_mut().iter_mut().enumerate() {
            b.copy_alu_src_if_cbuf(src, gpr, src_types[i]);
        }
    }
    ...
    match &mut instr.op {
        ...
        Op::BMsk(op) => {
            b.copy_alu_src_if_not_reg(&mut op.pos, gpr, SrcType::ALU);
        }
        Op::BRev(_) | Op::Flo(_) => (),
        Op::IAbs(_) => (),
        Op::IAdd3(op) => {
        ...
        Op::IAdd3X(op) => {
            ...
            if !op.is_uniform() {
                b.copy_src_if_upred(&mut op.carry[0]);
                b.copy_src_if_upred(&mut op.carry[1]);
            }
        }
        Op::IDp4(op) => {
        ...
        Op::ISetP(op) => {
            ...
            b.copy_alu_src_if_not_reg(src0, gpr, SrcType::ALU);
            if !op.is_uniform() {
                b.copy_src_if_upred(&mut op.low_cmp);
                b.copy_src_if_upred(&mut op.accum);
            }
        }
        Op::Lop3(op) => {
        ...
        Op::PopC(_) | Op::R2UR(_) => (),
        Op::Shf(op) => {
            b.copy_alu_src_if_not_reg(&mut op.low, gpr, SrcType::ALU);
        ...
        Op::Sel(op) => {
            if !op.is_uniform() {
                b.copy_src_if_upred(&mut op.cond);
            }
            let [ref mut src0, ref mut src1] = op.srcs;
            if swap_srcs_if_not_reg(src0, src1, gpr) {
                op.cond = op.cond.bnot();
